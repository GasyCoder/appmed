<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="min-w-0">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Enseignants</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Créer et gérer les comptes enseignants.</p>
        </div>

        <button type="button"
                wire:click="openCreateModal"
                class="inline-flex h-10 items-center justify-center rounded-xl bg-indigo-600 px-4 text-sm font-semibold
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <div>
            <select wire:model.live="niveau_filter"
                    class="h-10 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-900
                           focus:border-indigo-500 focus:ring-indigo-500
                           dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-gray-700 dark:bg-gray-800/50 dark:text-gray-200">
                    <tr>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    @forelse($teachers as $t)
                        <tr class="text-gray-800 dark:text-gray-100">
                            ...
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $t->email }}</td>
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-1">
                                    @foreach($t->teacherNiveaux as $nv)
                                        <span class="rounded-full border border-gray-200 bg-gray-50 px-2 py-1 text-xs
                                                     text-gray-700 dark:border-gray-800 dark:bg-gray-800 dark:text-gray-200">
                                            {{ $nv->name }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <td class="px-4 py-3">
                                <button type="button"
                                        wire:click="toggleUserStatus({{ $t->id }})"
                                        class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold
                                            {{ $t->status ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200'
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-end gap-2">
                                    <button type="button"
                                            wire:click="editTeacher({{ $t->id }})"
                                            class="inline-flex h-9 items-center justify-center rounded-xl border border-gray-200
                                                   bg-white px-3 text-xs font-semibold text-gray-700 hover:bg-gray-50
                                    ...
                                    <button type="button"
                                            ...
                                            class="inline-flex h-9 items-center justify-center rounded-xl border border-red-200
                                                   bg-red-50 px-3 text-xs font-semibold text-red-700 hover:bg-red-100
                                                   dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-200 dark:hover:bg-red-900/30 transition">
                                        Supprimer
                                    </button>
                                </div>
                            </td>
