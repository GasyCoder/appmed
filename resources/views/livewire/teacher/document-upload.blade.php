...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden"
    ...
    <div class="p-6">
        ...
        <div class="relative border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300"
             :class="{
                 'border-gray-300 dark:border-gray-600 hover:border-blue-400 hover:bg-blue-50/30 dark:hover:bg-blue-900/10': !isDragging && !$wire.isUploading && !showFileLoader,
                 ...
                 'border-amber-500 bg-amber-50 dark:bg-amber-900/20': showFileLoader
             }"
            ...
            <div @click="$refs.fileInput.click()" class="cursor-pointer">
                ...
                <div class="mb-6">
                    <div class="relative inline-block">
                        <svg class="w-20 h-20 mx-auto transition-all duration-300"
                             :class="{
                                 'text-gray-400 dark:text-gray-500': !isDragging && !$wire.isUploading && !showFileLoader,
                                 'text-blue-500 scale-110': isDragging,
                                 'text-green-500 animate-pulse': $wire.isUploading,
                                 'text-amber-500 animate-bounce': showFileLoader
                             }"
                        ...
                        <!-- Badge de loading files -->
                        <div x-show="showFileLoader" 
                             class="absolute -top-2 -right-2 w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center">
                            <svg class="w-4 h-4 text-white animate-spin" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                ...
                <div class="space-y-3">
                    <h3 class="text-2xl font-bold transition-colors duration-300"
                        :class="{
                            'text-gray-900 dark:text-white': !isDragging && !$wire.isUploading && !showFileLoader,
                            'text-blue-600': isDragging,
                            'text-green-600': $wire.isUploading,
                            'text-amber-600': showFileLoader
                        }">
                        <span x-show="!isDragging && !$wire.isUploading && !showFileLoader">Sélectionnez vos fichiers</span>
                        <span x-show="isDragging">Déposez vos fichiers ici</span>
                        <span x-show="$wire.isUploading">Conversion en cours...</span>
                        <span x-show="showFileLoader">Chargement des fichiers...</span>
                    </h3>
                    
                    <p class="text-gray-600 dark:text-gray-400 text-lg">
                        <span x-show="!$wire.isUploading && !showFileLoader">Glissez-déposez ou cliquez pour choisir</span>
                        <span x-show="$wire.isUploading">Traitement et conversion des fichiers</span>
                        <span x-show="showFileLoader">Veuillez patienter pendant le traitement</span>
                    </p>
                    
                    <!-- Formats supportés avec icônes -->
                    <div x-show="!showFileLoader && !$wire.isUploading" class="flex flex-wrap items-center justify-center gap-6 text-sm text-gray-500 dark:text-gray-400 mt-6">
                    ...
                    <div x-show="!showFileLoader && !$wire.isUploading" class="text-xs text-gray-400 dark:text-gray-500 mt-4">
        ...
        <div x-show="showFileLoader" 
             ...
             class="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border border-blue-200 dark:border-blue-700 p-6">
        ...
        <div x-show="!showFileLoader" 
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="button" 
                        wire:click="$set('file', [])"
                        class="text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 font-medium hover:bg-red-50 dark:hover:bg-red-900/20 px-3 py-1 rounded-lg transition-all">
                    Tout supprimer
                </button>
            </div>
...
function modernUploadZone() {
    return {
        ...
        init() {
            // Écouter le changement de fichiers
            this.$refs.fileInput.addEventListener('change', () => {
                if (this.$refs.fileInput.files.length > 0) {
                    this.showFileLoader = true;
                    console.log('Showing file loader...');
                }
            });

            // Écouter les événements Livewire existants
            ...
            // Cacher le loader quand les fichiers sont traités
            this.$wire.on('files-updated', (event) => {
                setTimeout(() => {
                    this.showFileLoader = false;
                    console.log('Hiding file loader...');
                }, 800); // Petit délai pour l'animation
            });
        }
    }
}
