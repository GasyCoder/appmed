<div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6"
    ...
    {{-- Fullscreen Loading Overlay : seulement pendant la soumission --}}
    <div
        wire:loading.flex
        ...
        <div class="rounded-2xl border border-gray-200 bg-white px-6 py-5 shadow-xl dark:border-gray-700 dark:bg-gray-800">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-bold text-gray-900 dark:text-white">Enregistrement…</p>
    ...
    @error('upload')
        <div class="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-red-900 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-emerald-200">
                    ...
                    <div x-show="source === 'local'" x-cloak class="mt-4"
                         x-data="{
                             ...
                             drop(e) {
                                ...
                                // Preview instantané (côté navigateur)
                                window.dispatchEvent(new CustomEvent('upload-preview', { detail: { files: dt.files }}));
                        ...
                        <input x-ref="fileInput"
                               ...
                               class="hidden"
                               id="fileInput"
                               @change="window.dispatchEvent(new CustomEvent('upload-preview', { detail: { files: $event.target.files }}))">
                        ...
                        {{-- Loader léger pendant upload tmp --}}
                ...
                <div class="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800">
                    ...
                    <div class="px-3 py-3">
                        ...
                        @if($source === 'local')
                            ...
                            {{-- (A) Preview instantané côté navigateur pendant upload tmp --}}
                            <div wire:loading.delay wire:target="files">
                                <template x-if="clientFiles.length > 0">
                                    <div class="space-y-2.5 max-h-[calc(100vh-18rem)] overflow-y-auto pr-1">
                                        <template x-for="(it, idx) in clientFiles" :key="'client-' + idx">
                                            <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/30 p-3">
                                                <div class="flex items-start gap-2">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate" x-text="it.name"></p>
                                                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                                            <span class="uppercase font-mono" x-text="it.ext"></span>
                                                            <span class="mx-1">•</span>
                                                            <span x-text="it.sizeKb + ' KB'"></span>
                                                            <template x-if="it.willConvert">
                                                                <span>
                                                                    <span class="mx-1">•</span>
                            ...
                            {{-- (B) Liste Livewire normale quand upload tmp terminé --}}
                                    ...
                                    <div class="space-y-2.5 max-h-[calc(100vh-18rem)] overflow-y-auto pr-1">
                                        @foreach($files as $index => $file)
                                            @php
                                                ...
                                                $sizeKb = round(($file->getSize() ?: 0) / 1024, 1);
                                            @endphp
    ...
    {{-- Alpine helper (preview instantané) --}}
    <script>
        function uploadPreview() {
            return {
                clientFiles: [],
                init() {
                    window.addEventListener('upload-preview', (e) => {
                        const files = e?.detail?.files ? Array.from(e.detail.files) : [];
                        this.clientFiles = files.map(f => {
                            const ext = (f.name.split('.').pop() || '').toLowerCase();
                            return {
                                name: f.name,
                                ext: ext,
                                sizeKb: Math.round((f.size / 1024) * 10) / 10,
                                willConvert: ['doc','docx','ppt','pptx'].includes(ext),
                            };
                        });
                    });
                }
